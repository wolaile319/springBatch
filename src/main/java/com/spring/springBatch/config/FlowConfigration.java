   ...
   public Step flowStep1(){
       return  stepBuilderFactory.get("flowStep1").tasklet(new Tasklet() {
           ...
           public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
               System.out.println("这是flowStep1");
    ...
    public Step flowStep2(){
        return  stepBuilderFactory.get("flowStep2").tasklet(new Tasklet() {
            @Override
            public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
                System.out.println("这是flowStep2");
                return RepeatStatus.FINISHED;
            }
        }).build();
    }
    ...
    public Step flowStep3(){
        return  stepBuilderFactory.get("flowStep3").tasklet(new Tasklet() {
            @Override
            public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
                System.out.println("这是flowStep3");
                return RepeatStatus.FINISHED;
            }
        }).build();
    }
    ...
    public Flow flowDemo(){
        return new FlowBuilder<Flow>("flowDemo").start(flowStep1()).next(flowStep2()).end();
    }

    @Bean
    public Job flowJob(){
        return  jobBuilderFactory.get("flowStep1").start(flowDemo()).next(flowStep3()).end().build();
    }
}
